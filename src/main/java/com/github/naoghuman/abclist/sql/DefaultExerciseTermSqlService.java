 ...
 * @author Naoghuman
 */
final class DefaultExerciseTermSqlService implements IDefaultConfiguration, IExerciseTermConfiguration, ExerciseTermSqlService {
    
    DefaultExerciseTermSqlService() {
        this.initialize();
    }
    
    private void initialize() {
        // Create a [dummy] ExerciseTerm - so the table exists in the database
        // TODO should only done if the table doesn't exists
        this.createExerciseTerm(new ExerciseTerm());
    }
    ...
    @Override
    public int deleteAllExerciseTermsWithExerciseId(long exerciseId) {
    ...
    @Override
    public ObservableList<Term> findAllTermsInExerciseTermsWithoutParent(ObservableList<Term> terms) {
        final ObservableList<Term> allTermsWithOutParent = FXCollections.observableArrayList();
        
        long counterTermInExercise;
        for (Term term : terms) {
            counterTermInExercise = this.countAllExerciseTermsWithTermId(term.getId());
